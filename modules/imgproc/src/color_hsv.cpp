...
struct HSV2RGB_b
{
    ...
    #if CV_SIMD128
    inline void process_simd(const uchar* src, v_float32x4* v_src) const
    {
        v_load_deinterleave_expand(src, v_src[0], v_src[1], v_src[2], v_src[3], v_src[4], v_src[5]);
        ...
        v_src[5] *= v_coeff0;
        HSV2RGB_simd(v_src[0], v_src[1], v_src[2], hscale);
        HSV2RGB_simd(v_src[3], v_src[4], v_src[5], hscale);
